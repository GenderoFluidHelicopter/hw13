...
def get_dataloader(x, y, batch_size=32, shuffle=False):
    ...
    for _ in range(1 if shuffle else 1):
        for start in range(0, n, batch_size):
            ...
            yield solevoy_tor4.tensor(x[sl]), solevoy_tor4.tensor(y[sl])
...
def ohe(y):
    ...
    y_zeros = np.zeros((len(y), y.max() + 1))
...
class LinearModel:
    def __init__(self, in_features, n_classes):
        ...
        # TODO
        self.weight = solevoy_tor4.Tensor(data=np.random.normal(size=(in_features, n_classes)), requires_grad=True)
    ...
    def forward(self, x):
        ...
        # x: [b, f]
        # TODO
        return solevoy_tor4.matmul(x, self.weight).sigmoid()
...
def get_criterion():
    def criterion(logits, labels):
        ...
        # TODO
        return solevoy_tor4.mse_loss(labels, logits)

    return criterion


def get_optimizer(params, lr=1e-3):
    opt = solevoy_tor4.SGD(params, lr=lr)
